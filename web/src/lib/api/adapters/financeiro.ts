...
function linhaFoco(r: Row): FocoLinha {
  return {
    ...
    documento: clean(r.numeroDocumento),
    dias: num(r.dias),
...
export function adaptFinanceiro(
  ...
  // inclui 'Hoje') E em hoje_vencidos — dedupe por numeroDocumento|DtVcto|VlrTitulo.
  ...
  for (const r of [...bolVencidos, ...bolAVencer, ...carVencidos, ...carAVencer, ...hojeVencidos]) {
    ...
    if (!data) continue;
    const chave = `${clean(r.numeroDocumento)}|${data}|${num(r.VlrTitulo)}`;
